...
def nested_tscv_ridge_cuda(X_train, y_train, alphas, device, n_splits=5):
    """
    Perform Ridge Regression for multiple alphas using TimeSeriesSplit cross-validation
    ...
    Parameters:
        ...
        n_splits (int): Number of folds for time series cross-validation (default 5).
    ...
    tscv = TimeSeriesSplit(n_splits=n_splits)  # Time-aware cross-validation
    ...
    for j, alpha in enumerate(alphas):
        ...
        for train_idx, val_idx in tscv.split(X_train.cpu()):  # Get time-ordered train/val indices
            # Split into training and validation sets
            X_train_fold, X_val_fold = X_train[train_idx], X_train[val_idx]
            y_train_fold, y_val_fold = y_train[train_idx], y_train[val_idx]

            # Compute (X^T X + alpha*I)
            XtX = torch.mm(X_train_fold.t(), X_train_fold)
            reg_term = alpha * torch.eye(X_train_fold.shape[1], device=device)
            Xty = torch.mm(X_train_fold.t(), y_train_fold.unsqueeze(1))

            # Solve for ridge regression weights
            weights, _, _, _ = torch.linalg.lstsq(XtX + reg_term, Xty)

            # Compute predictions on validation set
            y_pred = torch.mm(X_val_fold, weights)

            # Compute Mean Squared Error (MSE)
            mse = torch.nn.functional.mse_loss(y_pred, y_val_fold.unsqueeze(1)).item()
            mse_folds.append(mse)

        # Compute mean MSE across all TimeSeriesSplit folds
        mse_alphas[j] = sum(mse_folds) / n_splits
...
def time_series_ridge_cv_cuda(X, y, n_splits=5, device='cuda'):
    ...
    for fold, (train_idx, test_idx) in enumerate(tscv.split(X_scaled)):
        ...
        # Find best alpha using nested "LOOCV", true LOOCV will cost a lot of time, thus using 5-folds instead
        best_alpha = nested_tscv_ridge_cuda(X_train, y_train, alphas, device)
